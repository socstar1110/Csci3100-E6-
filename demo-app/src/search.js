...
const Search =() =>{
    ...
    function SerachByConditon(event){
      ...
      },body: JSON.stringify((serachByConditon)) // send modifyCourse object to backend
      })
      .then(res => res.text())
      ...
      <div> {/* show the button on top right corner*/}
        ...
        <div className = "container d-flex justify-content-center align-items-center" >
          <div className="search" style={{ display: 'block' }}> 
            ...
            <button style={{ width: '30px', height: '30px', padding: '0px' }} onClick={SerachByConditon}> 
              <SearchIcon style={{ width: '25px', height: '25px', padding: '0px' }}/> 
            </button>
          </div>
        </div>
        }
